...
class Contract(sp.Contract):
  def __init__(self):
    ...
    self.init(admin = sp.address('tz1aSkwEot3L2kmUvcoxzjMomb9mvBNuzFK6'),
              isPoolRegistered = {},
  ...
  def batchSwap(self, params):
    ...
    compute_Swaps_147 = sp.local("compute_Swaps_147", {}, sp.TMap(sp.TNat, sp.TInt))
    ...
    sp.for i in sp.range(0, sp.len(params.swaps)):
      ...
      sp.if params.swaps[i].amount == 0:
        sp.verify(i > 0, 510)
        compute_Swaps_168 = sp.local("compute_Swaps_168", sp.eif(params.kind == 'GIVEN_IN', params.assets[params.swaps[i].assetInIndex], params.assets[params.swaps[i].assetOutIndex]))
        sp.verify(previousTokenCalculated.value == compute_Swaps_168.value, 511)
      ...
      compute_Swaps_263 = sp.local("compute_Swaps_263", sp.eif(params.kind == 'GIVEN_IN', (amount.value, compute_Swaps_260.value), (compute_Swaps_260.value, amount.value)))
      match_pair_Swaps_263_fst, match_pair_Swaps_263_snd = sp.match_tuple(compute_Swaps_263.value, "match_pair_Swaps_263_fst", "match_pair_Swaps_263_snd")
      ...
      self.data.poolsBalances[params.swaps[i].poolId][params.assets[params.swaps[i].assetOutIndex]] = (sp.as_nat(sp.fst(self.data.poolsBalances.get(params.swaps[i].poolId, message = 500).get(params.assets[params.swaps[i].assetOutIndex], message = 521)) - match_pair_Swaps_263_snd), sp.snd(self.data.poolsBalances.get(params.swaps[i].poolId, message = 500).get(params.assets[params.swaps[i].assetOutIndex], message = 521)))
      match_pair_Swaps_207_fst, match_pair_Swaps_207_snd = sp.match_tuple(sp.eif(params.kind == 'GIVEN_IN', (amount.value, compute_Swaps_260.value), (compute_Swaps_260.value, amount.value)), "match_pair_Swaps_207_fst", "match_pair_Swaps_207_snd")
      ...
      previousAmountCalculated.value = compute_Swaps_260.value
      compute_Swaps_188 = sp.local("compute_Swaps_188", sp.eif(params.kind == 'GIVEN_IN', params.assets[params.swaps[i].assetOutIndex], params.assets[params.swaps[i].assetInIndex]))
      previousTokenCalculated.value = compute_Swaps_188.value
      compute_Swaps_147.value[params.swaps[i].assetInIndex] = compute_Swaps_147.value.get(params.swaps[i].assetInIndex, default_value = 0) + sp.to_int(match_pair_Swaps_207_fst)
      compute_Swaps_147.value[params.swaps[i].assetOutIndex] = compute_Swaps_147.value.get(params.swaps[i].assetOutIndex, default_value = 0) - sp.to_int(match_pair_Swaps_207_snd)
    sp.for i in sp.range(0, sp.len(params.assets)):
      sp.verify(compute_Swaps_147.value[i] <= params.limits[i], 507)
      sp.if compute_Swaps_147.value[i] > 0:
        sp.verify(sp.as_nat(compute_Swaps_147.value[i]) > 0, 'Zero_Transfer')
        ...
        sp.else:
          sp.transfer(sp.record(from_ = params.funds.sender, to_ = sp.self_address, value = sp.as_nat(compute_Swaps_147.value[i])), sp.tez(0), sp.contract(sp.TRecord(from_ = sp.TAddress, to_ = sp.TAddress, value = sp.TNat).layout(("from_ as from", ("to_ as to", "value"))), sp.fst(params.assets[i]), entrypoint='transfer').open_some())
      sp.if compute_Swaps_147.value[i] < 0:
        sp.verify((abs(compute_Swaps_147.value[i])) > 0, 'Zero_Transfer')
        sp.if sp.snd(params.assets[i]) != sp.none:
        ...
        sp.else:
          sp.transfer(sp.record(from_ = sp.self_address, to_ = params.funds.recipient, value = abs(compute_Swaps_147.value[i])), sp.tez(0), sp.contract(sp.TRecord(from_ = sp.TAddress, to_ = sp.TAddress, value = sp.TNat).layout(("from_ as from", ("to_ as to", "value"))), sp.fst(params.assets[i]), entrypoint='transfer').open_some())

  @sp.entrypoint
  def exitPool(self, params):
    compute_PoolBalances_120 = sp.local("compute_PoolBalances_120", self._validateTokensAndGetBalances(sp.record(expectedTokens = params.request.assets, limits = params.request.limits, poolId = params.poolId)))
    compute_BalanceAllocation_5 = sp.local("compute_BalanceAllocation_5", {}, sp.TMap(sp.TNat, sp.TNat))
    sp.for i in sp.range(0, sp.len(compute_PoolBalances_120.value)):
      compute_BalanceAllocation_5.value[i] = sp.fst(compute_PoolBalances_120.value[i]) + sp.snd(compute_PoolBalances_120.value[i])
    compute_PoolBalances_132 = sp.local("compute_PoolBalances_132", sp.view("beforeExitPool", sp.record(balances = compute_BalanceAllocation_5.value, userData = params.request.userData), sp.fst(params.poolId), sp.TTuple(sp.TNat, sp.TMap(sp.TNat, sp.TNat), sp.TNat)).open_some(message = 'Invalid view'))
    sptAmountIn, amountsOut, invariant = sp.match_tuple(compute_PoolBalances_132.value, "sptAmountIn", "amountsOut", "invariant")
    ...
    compute_PoolBalances_210 = sp.local("compute_PoolBalances_210", {}, sp.TMap(sp.TNat, sp.TPair(sp.TNat, sp.TNat)))
    sp.for x in sp.range(0, sp.len(params.request.assets)):
      ...
      compute_PoolBalances_210.value[x] = (sp.as_nat(sp.fst(compute_PoolBalances_120.value[x]) - amountsOut[x]), sp.snd(compute_PoolBalances_120.value[x]))
    sp.for i in sp.range(0, sp.len(params.request.assets)):
      self.data.poolsBalances[params.poolId][params.request.assets[i]] = compute_PoolBalances_210.value[i]
  ...
  def joinPool(self, params):
    compute_PoolBalances_61 = sp.local("compute_PoolBalances_61", self._validateTokensAndGetBalances(sp.record(expectedTokens = params.request.assets, limits = params.request.limits, poolId = params.poolId)))
    compute_BalanceAllocation_5 = sp.local("compute_BalanceAllocation_5", {}, sp.TMap(sp.TNat, sp.TNat))
    sp.for i in sp.range(0, sp.len(compute_PoolBalances_61.value)):
      compute_BalanceAllocation_5.value[i] = sp.fst(compute_PoolBalances_61.value[i]) + sp.snd(compute_PoolBalances_61.value[i])
    compute_PoolBalances_72 = sp.local("compute_PoolBalances_72", sp.view("beforeJoinPool", sp.record(balances = compute_BalanceAllocation_5.value, userData = params.request.userData), sp.fst(params.poolId), sp.TTuple(sp.TNat, sp.TMap(sp.TNat, sp.TNat), sp.TNat)).open_some(message = 'Invalid view'))
    sptAmountOut, amountsIn, invariant = sp.match_tuple(compute_PoolBalances_72.value, "sptAmountOut", "amountsIn", "invariant")
    ...
    sp.verify(sp.len(compute_PoolBalances_61.value) == sp.len(amountsIn))
    compute_PoolBalances_182 = sp.local("compute_PoolBalances_182", {}, sp.TMap(sp.TNat, sp.TPair(sp.TNat, sp.TNat)))
    sp.for x in sp.range(0, sp.len(params.request.assets)):
      ...
      compute_PoolBalances_182.value[x] = (sp.fst(compute_PoolBalances_61.value[x]) + amountsIn[x], sp.snd(compute_PoolBalances_61.value[x]))
    sp.for i in sp.range(0, sp.len(params.request.assets)):
      self.data.poolsBalances[params.poolId][params.request.assets[i]] = compute_PoolBalances_182.value[i]
  ...
  def registerPool(self):
    sp.verify(self.data.settings.paused == False, 'ONLY_UNPAUSED')
    compute_PoolRegistry_18 = sp.local("compute_PoolRegistry_18", self.data.nextPoolNonce)
    self.data.isPoolRegistered[(sp.sender, compute_PoolRegistry_18.value)] = sp.unit
    self.data.nextPoolNonce += 1
    sp.emit(sp.record(pool = sp.sender, poolId = (sp.sender, compute_PoolRegistry_18.value)), tag = "PoolRegistered")
  ...
  def swap(self, params):
    ...
    compute_Swaps_263 = sp.local("compute_Swaps_263", sp.eif(params.singleSwap.kind == 'GIVEN_IN', (params.singleSwap.amount, compute_Swaps_260.value), (compute_Swaps_260.value, params.singleSwap.amount)))
    match_pair_Swaps_263_fst, match_pair_Swaps_263_snd = sp.match_tuple(compute_Swaps_263.value, "match_pair_Swaps_263_fst", "match_pair_Swaps_263_snd")
    ...
    self.data.poolsBalances[params.singleSwap.poolId][params.singleSwap.assetOut] = (sp.as_nat(sp.fst(self.data.poolsBalances.get(params.singleSwap.poolId, message = 500).get(params.singleSwap.assetOut, message = 521)) - match_pair_Swaps_263_snd), sp.snd(self.data.poolsBalances.get(params.singleSwap.poolId, message = 500).get(params.singleSwap.assetOut, message = 521)))
    match_pair_Swaps_207_fst, match_pair_Swaps_207_snd = sp.match_tuple(sp.eif(params.singleSwap.kind == 'GIVEN_IN', (params.singleSwap.amount, compute_Swaps_260.value), (compute_Swaps_260.value, params.singleSwap.amount)), "match_pair_Swaps_207_fst", "match_pair_Swaps_207_snd")
    sp.emit(sp.record(amountIn = match_pair_Swaps_207_fst, amountOut = match_pair_Swaps_207_snd, poolId = params.singleSwap.poolId, tokenIn = (sp.fst(params.singleSwap.assetIn), sp.snd(params.singleSwap.assetIn)), tokenOut = (sp.fst(params.singleSwap.assetOut), sp.snd(params.singleSwap.assetOut))), tag = "Swap")
    sp.verify(sp.eif(params.singleSwap.kind == 'GIVEN_IN', match_pair_Swaps_207_snd >= params.limit, match_pair_Swaps_207_fst <= params.limit), 507)
    sp.verify(match_pair_Swaps_207_fst > 0, 'Zero_Transfer')
    ...
    sp.else:
      sp.transfer(sp.record(from_ = params.funds.sender, to_ = sp.self_address, value = match_pair_Swaps_207_fst), sp.tez(0), sp.contract(sp.TRecord(from_ = sp.TAddress, to_ = sp.TAddress, value = sp.TNat).layout(("from_ as from", ("to_ as to", "value"))), sp.fst(params.singleSwap.assetIn), entrypoint='transfer').open_some())
    sp.verify(match_pair_Swaps_207_snd > 0, 'Zero_Transfer')
    ...
    sp.else:
      sp.transfer(sp.record(from_ = sp.self_address, to_ = params.funds.recipient, value = match_pair_Swaps_207_snd), sp.tez(0), sp.contract(sp.TRecord(from_ = sp.TAddress, to_ = sp.TAddress, value = sp.TNat).layout(("from_ as from", ("to_ as to", "value"))), sp.fst(params.singleSwap.assetOut), entrypoint='transfer').open_some())
  ...
  @sp.private_lambda()
  def _castToInt(_x0):
    compute_PoolBalances_240 = sp.local("compute_PoolBalances_240", {})
    sp.for i in sp.range(0, sp.len(_x0.amounts)):
      sp.if _x0.positive:
        compute_PoolBalances_240.value[i] = sp.to_int(_x0.amounts[i])
      sp.else:
        compute_PoolBalances_240.value[i] = - sp.to_int(_x0.amounts[i])
    sp.result(compute_PoolBalances_240.value)

  @sp.private_lambda()
  def _validateTokensAndGetBalances(_x2):
    sp.verify(sp.len(_x2.expectedTokens) == sp.len(_x2.limits))
    compute_PoolTokens_91 = sp.local("compute_PoolTokens_91", {}, sp.TMap(sp.TNat, sp.TPair(sp.TAddress, sp.TOption(sp.TNat))))
    compute_PoolTokens_92 = sp.local("compute_PoolTokens_92", {}, sp.TMap(sp.TNat, sp.TPair(sp.TNat, sp.TNat)))
    sp.for i in sp.range(0, sp.len(self.data.poolsTokens[_x2.poolId])):
      compute_PoolTokens_91.value[i] = self.data.poolsTokens[_x2.poolId][i]
      compute_PoolTokens_92.value[i] = self.data.poolsBalances.get(_x2.poolId, default_value = {}).get(self.data.poolsTokens[_x2.poolId][i], default_value = (0, 0))
    sp.verify(sp.len(compute_PoolTokens_91.value) == sp.len(_x2.expectedTokens))
    sp.verify(sp.len(compute_PoolTokens_91.value) > 0, 527)
    sp.for i in sp.range(0, sp.len(compute_PoolTokens_91.value)):
      sp.verify(compute_PoolTokens_91.value[i] == _x2.expectedTokens[i], 520)
    sp.result(compute_PoolTokens_92.value)
